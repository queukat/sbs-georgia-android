...
class TbcStatementParserTest {
    ...
    fun handlesWhitespaceVariantsWithoutLosingSuggestionQuality() {
        ...
        assertEquals(
            ...
            preview.rows.first {
                it.description.equals("bank fee", ignoreCase = true)
            }.suggestedInclusion
        )
        assertEquals(
            DeclarationInclusion.EXCLUDED,
            preview.rows.first {
                it.description.equals("Own Account Transfer", ignoreCase = true)
            }.suggestedInclusion
        )
        assertEquals(
            DeclarationInclusion.REVIEW_REQUIRED,
            preview.rows.first {
                it.description.equals("Client Payment", ignoreCase = true)
            }.suggestedInclusion
        )
    }
    ...
    fun parsesGeorgianHeadersAndKeepsSafeSuggestionQuality() {
        ...
        assertEquals(
            DeclarationInclusion.EXCLUDED,
            preview.rows.first {
                it.description == "საკუთარ ანგარიშებს შორის გადარიცხვა"
            }.suggestedInclusion
        )
        assertEquals(
            DeclarationInclusion.REVIEW_REQUIRED,
    ...
    fun parsesRealCollapsedBilingualStatementExport() {
        ...
        val januaryFee =
            preview.rows.first {
                it.incomeDate.toString() == "2026-01-28" &&
                    it.description.contains("მომსახურების საკომისიო")
            }
