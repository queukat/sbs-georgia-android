...
class StatementImportUseCasesTest {
    ...
    fun confirmImportAutomaticallyResolvesFxForAffectedMonths() = kotlinx.coroutines.test.runTest {
        ...
        val fxRateRepository =
            StatementImportFakeFxRateRepository(
                cachedRates =
                mapOf(
                    (LocalDate.of(2026, 3, 15) to "USD") to
                        FxRate(
    ...
    fun confirmImportFlagsTreasuryTaxPaymentForManualReview() = kotlinx.coroutines.test.runTest {
        val result =
            ConfirmStatementImportUseCase(
                statementImportRepository = FakeStatementImportRepository(),
                resolveFxForMonthsUseCase = resolveFxForMonthsUseCase(
                    StatementImportFakeIncomeRepository()
                ),
                detectImportedTaxPaymentCandidatesUseCase = DetectImportedTaxPaymentCandidatesUseCase(),
                clock = fixedClock
            ).invoke(
    ...
    fun loadPreviewHandlesOverlappingStatementWindowByFlaggingOnlySharedRows() = kotlinx.coroutines.test.runTest {
        val alreadyImportedFingerprint =
            TbcStatementParser()
                .parse(
                    "statement.pdf",
                    "fingerprint",
        ...
        assertTrue(
            previewRows.first {
                it.description == "FOR SOFTWARE SERVICES" &&
                    it.additionalInformation == "Invoice 001"
            }.duplicate
        )
        assertTrue(
            previewRows.first {
                it.additionalInformation == "Invoice 002"
            }.duplicate.not()
        )
    }
    ...
    private fun fixtureText(fileName: String): String = requireNotNull(javaClass.getResource("/fixtures/$fileName"))
        .readText()
...
private class FakeStatementDocumentReader(private val document: ImportedPdfDocument) : StatementDocumentReader {
...
private class StatementImportFakeIncomeRepository(private val entries: MutableList<IncomeEntry> = mutableListOf()) :
    IncomeRepository {
