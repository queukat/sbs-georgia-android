...
data class MonthFxResolutionResult(
    val resolvedEntryCount: Int,
    val unresolvedEntryCount: Int,
    val message: String,
)

class ResolveMonthFxUseCase @Inject constructor(
    ...
    suspend operator fun invoke(entries: List<IncomeEntry>): MonthFxResolutionResult {
        ...
        val groups = unresolvedEntries.groupBy { it.incomeDate to it.originalCurrency.uppercase() }
        var resolvedCount = 0
        ...
        groups.forEach { (_, groupedEntries) ->
            ...
            val resolvedRate = fxRateRepository.getBestRate(sampleEntry.incomeDate, sampleEntry.originalCurrency)
                ?: when (val fetchedRate = fxRateRepository.fetchOfficialRate(sampleEntry.incomeDate, sampleEntry.originalCurrency)) {
            ...
            if (resolvedRate == null) {
                ...
                groupedEntries.forEach { entry ->
                    incomeRepository.upsert(entry.applyFxRate(resolvedRate.units, resolvedRate.rateToGel, resolvedRate.source, resolvedRate.manualOverride, clock))
                    resolvedCount += 1
                }
            }
        }
...
class ApplyManualFxOverrideUseCase @Inject constructor(
    private val fxRateRepository: FxRateRepository,
    private val incomeRepository: IncomeRepository,
    private val clock: Clock,
) {
    suspend operator fun invoke(
        ...
        val entry = requireNotNull(incomeRepository.getById(entryId)) {
            "Income entry $entryId was not found."
        }
        val fxRate = fxRateRepository.upsertManualOverride(
            rateDate = entry.incomeDate,
            currencyCode = entry.originalCurrency,
            units = units,
            rateToGel = rateToGel,
        )
        val updatedEntry = entry.applyFxRate(
            units = fxRate.units,
