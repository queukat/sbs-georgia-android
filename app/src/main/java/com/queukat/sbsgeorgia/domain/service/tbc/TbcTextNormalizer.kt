...
internal fun sanitizeLogicalLine(line: String): String {
    ...
    TbcStatementFormat.inlineArtifactRegexes.forEach { regex ->
        ...
        if (markerIndex >= TbcStatementFormat.DATE_TOKEN_LENGTH &&
            hasTrailingAmount(trimmed.substring(0, markerIndex))
        ) {
...
internal object TbcStatementFormat {
    ...
    const val DATE_TOKEN_LENGTH = 10
    ...
    val statementPeriodRegex =
        Regex("^ge\\d{2}[a-z0-9]+.*\\d{2}/\\d{2}/\\d{4}\\s*-\\s*\\d{2}/\\d{2}/\\d{4}.*$")
    val openingBalanceRegex =
        Regex(
            "(?:საწყისი ნაშთი|Opening Balance).*?([0-9][0-9,]*\\.\\d{2})\\s*([A-Z]{3})",
    ...
    val headerHints =
        listOf(
            ...
            "date / თარიღი description / დანიშნულება additional information / დამატებითი ინფორმაცია " +
                "paid out / გასული თანხა paid in / შემოსული თანხა balance / ბალანსი"
        )
    val inlineArtifactRegexes =
        listOf(
            Regex(
                "\\b\\d+\\s*-\\s*\\d+\\b\\s+(?=(?:account statement|statement from account|" +
                    "ამონაწერი ანგარიშიდან|ანგარიშის ამონაწერი|account holder|ანგარიშის მფლობელი|" +
                    "date\\b|თარიღი\\b))",
                RegexOption.IGNORE_CASE
            ),
            Regex(
                "(?=account statement|statement from account|ამონაწერი ანგარიშიდან|" +
                    "ანგარიშის ამონაწერი|account holder|ანგარიშის მფლობელი|generated on|" +
                    "გენერირებულია|opening balance|closing balance)",
                RegexOption.IGNORE_CASE
            ),
            Regex(
                "(?=date\\s*/\\s*თარიღი|თარიღი\\s+დანიშნულება|date\\s+description)",
                RegexOption.IGNORE_CASE
            ),
            Regex(
                "\\bge\\d{2}[a-z0-9]+.*\\d{2}/\\d{2}/\\d{4}\\s*-\\s*\\d{2}/\\d{2}/\\d{4}.*",
                RegexOption.IGNORE_CASE
            )
        )
    val headerLineFragments =
        setOf(
            "თარიღი",
            "date",
            "დანიშნულება",
    ...
    val additionalInfoSplitMarkers =
        listOf(
            "დებიტორები -",
            "Debtors -"
        )
    val currencyDetectionRegexes =
        listOf(
