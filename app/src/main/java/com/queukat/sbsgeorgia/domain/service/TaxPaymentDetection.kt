...
object TaxPaymentDetection {
    ...
    fun matchesTaxPaymentText(
        description: String,
        additionalInformation: String?,
    ): Boolean {
        val normalized = listOf(description, additionalInformation.orEmpty())
            .joinToString(" ")
            .lowercase()
        return taxPaymentHints.any { it in normalized }
    }

    fun resolveOutgoingAmount(
        paidOut: StatementMoney?,
        fallbackAmount: BigDecimal,
    ): BigDecimal? = paidOut?.amount?.takeIf { it.signum() == 1 } ?: fallbackAmount.takeIf { it.signum() == 1 }

    private val taxPaymentHints = listOf(
        "tax unified code",
