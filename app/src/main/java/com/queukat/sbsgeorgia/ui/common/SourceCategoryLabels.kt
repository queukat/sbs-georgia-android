...
private val knownSourceCategories = listOf(
    SourceCategoryMapping(SourceCategoryPresets.SOFTWARE_SERVICES, R.string.source_category_software_services),
    SourceCategoryMapping(SourceCategoryPresets.CONSULTING, R.string.source_category_consulting),
    SourceCategoryMapping(SourceCategoryPresets.MARKETPLACE_PAYOUT, R.string.source_category_marketplace_payout),
...
fun displaySourceCategory(context: Context, rawValue: String): String =
    knownSourceCategories.firstOrNull { it.rawValue.equals(rawValue.trim(), ignoreCase = true) }
...
fun canonicalSourceCategory(context: Context, value: String): String {
    ...
    return knownSourceCategories.firstOrNull { mapping ->
